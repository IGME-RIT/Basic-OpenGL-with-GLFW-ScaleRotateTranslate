...
struct shape {
	...
	glm::vec3 rotation = glm::vec3(0.0f, 0.0f, 30.0f);			//Rotation in radians
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Translation Tutorial", nullptr, nullptr);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Detailed notes about the math ^
		//X Axis (pitch) where A is rotation around the X axis.
		//|1	0	       0|	
		//|0  cos(A) -sin(A)|	
		//|0  sin(A)  cos(A)|

		//Y Axis (yaw) where B is rotation around the Y axis.
		//|cos(B)	0  sin(B)|	
		//|0        1		0|
		//|-sin(B)  0  cos(B)|

		//Z Axis (Roll) where C is rotation around the Z axis.
		//|cos(C)	-sin(C)  0|	
		//|sin(C)    cos(C)	 0|
		//|0			0	 1|

		//GLMs yawPitchRoll() multiplies the above matrices in the Ry*Rx*Rz order
		//You could implement your own angle rotations as well
